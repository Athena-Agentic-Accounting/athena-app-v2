...
export function ArtifactPanel({
  ...
  const [internalActiveTabId, setInternalActiveTabId] = useState(artifact.activeTabId)
  const [openTabIds, setOpenTabIds] = useState(() => artifact.tabs.map((tab) => tab.id))
  ...
  function handlePlanDecision(decision: PlanReviewDecision) {
    ...
    if (decision === "reject" || decision === "start_now" || decision === "schedule") {
      setPlanDecided(true)
    }
  }
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="mx-auto max-w-3xl px-8 py-6 pb-24">
          {activeTab.kind === "document" ? (
            ...
            <PlanReviewContent markdown={activeTab.markdown} table={activeTab.table} />
          )}
        </div>
      </ScrollArea>

      {activeTab.kind === "plan_review" ? (
        <PlanReviewActions disabled={planDecided} onDecision={handlePlanDecision} />
      ) : (
